...
try:
    ...
    # from run_simulation import *#Remove the dependency of the grader on run_simulation.py (shouldn't then need casadi!)
    ...
    ****************************************
    ...
    try:
        ...
        vel1 = np.array([[0.1, 0.1, 0]]).T
        dynamics._x = np.vstack((np.zeros((3, 1)), vel1)) #MANUALLY overrride dynamics.
        ...
        passed = passed and np.allclose(RSoln, RStudent, rtol=1e-1)
    ...
    try:
        ...
        #check if the two are within tolerance.
        passed = passed and np.allclose(ptMatrixSoln, ptMatrixStudent, rtol=1e-1)
    ...
    try:
        ...
        depthCam.calc_ptcloud()
        
        #Now store the pointcloud and its KD tree     
        ptcloud = depthCam._ptcloudData["ptcloud"]
        ptcloud_KD = depthCam._ptcloudData["kd"]
        
        #Now call knn functions
        closest_K_soln = get_knn_soln(queueSize, ptcloud, ptcloud_KD)
        closest_K_soln = closest_K_soln.reshape((queueSize*3, 1))
        
        closest_K_student = depthCam.get_knn(queueSize)
        closest_K_student = closest_K_student.reshape((queueSize*3, 1))
                
        #check if the norms are close.
        passed = passed and (np.linalg.norm(closest_K_student - closest_K_soln) <= 1e-1)
        
        if passed:
            pointcloud_score = indiv_scores
            pointcloud_message = "Correct implementation of get_knn."
        else:
            pointcloud_score = 0
            pointcloud_message = "Incorrect implementation of get_knn."
    except Exception as e:
        message = f"Unable to run get_knn test. Following exception thrown: \n{e}"

except Exception as e:
    message = f"Unable to run simulation tests. Following exception thrown: \n{e}"


total_score = get_orient_score + depth_to_spatial_score + pointcloud_score
...
output = {
        ...
        "tests": [
            ...
            {
                "score": pointcloud_score,
                "max_score": indiv_scores,
                "name": "pointcloud test",
                "output": pointcloud_message,
                "visibility": "visible"
            }
        ]
        }
